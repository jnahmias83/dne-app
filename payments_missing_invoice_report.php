...
if($lang== 'EN') {
    $title = 'Project '.@$project->name."<br/>Missing Invoices Report";
    $dir_table = 'ltr';
	...
	$title = 'פרוייקט '.@$project->name_he."<br/>דו''ח חשבוניות חסרות";
    $dir_table = 'rtl';
...
$query = $mysqli->prepare("SELECT p.id AS id,p.id_projects_suppliers AS id_projects_suppliers,
						  ...
						  s.name_he AS s_name_he,s.email_office AS s_email_office,
						  s.mobile AS s_mobile,ps.id AS ps_id
...
class PDF extends TCPDF {
    public function Footer() { 
	    ...
	    $pdf_file_name = substr(date('Y-m-d'),0,4).substr(date('Y-m-d'),5,2).substr(date('Y-m-d'),8,2).'-Missing Invoices Report-'.$project_nickname.'.pdf';
...
$html.= '<table dir="'.$dir_table.'" style="'.$style_table.'"><tr><td width="5%">&nbsp;</td><td><table cellpadding="4">';
...
$html.='<th width="100px;" style="text-align:center;border:1px solid black;">'.getLang2($lang,'mobile').'</th>';
$html.='</tr>';
...
foreach($payments_missing_invoice as $item) { 
	...
	if(@$item->paid_amount_vat_included != 0.00){
		$amount = floatval(@$item->paid_amount_vat_included);
		$paid_amount_vat_included =
			(floor($amount) == $amount)
