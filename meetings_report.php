...
// (meme technique de mesure + espaceur plafonne que pour les lignes de donnees)
$header_cells_def = array();
$header_cells_def['count'] = array(@$count_width, '&#x2116;');
...
foreach ($header_cells_def as $hk => $hv) {
	$hw_mm = $pdf->getHTMLUnitToUnits($hv[0], $new_task_legend_page_w ?? ($pdf->getPageWidth() - $pdf->getMargins()['left'] - $pdf->getMargins()['right']), 'px');
