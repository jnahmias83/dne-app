...
if($lang== 'EN') {
    $title = 'Project '.@$project->name."<br/>Payments Report";
    $dir_table = 'ltr';
	...
	$title = 'פרוייקט '.@$project->name_he."<br/>דו''ח ביצוע תשלומים";
    $dir_table = 'rtl';
...
$query = $mysqli->prepare("SELECT p.id AS id,p.id_projects_suppliers AS id_projects_suppliers,p.description AS description,
						  ...
						  p.paid_amount_vat_excluded AS paid_amount_vat_excluded,p.invoice_date AS invoice_date,
						  p.pdf_invoice AS pdf_invoice,s.name AS s_name,s.name_he AS s_name_he				  
						  ...
						  WHERE ps.id_project = ? ORDER BY p.payment_date DESC");
$query->bind_param("i",$project_id);
$query->execute(); 
$query->store_result();
...
class PDF extends TCPDF {
    public function Footer() { 
	    ...
	    $pdf_file_name = substr(date('Y-m-d'),0,4).substr(date('Y-m-d'),5,2).substr(date('Y-m-d'),8,2).'-Payments Report-'.$project_nickname.'.pdf';
...
$html.='<th width="70px;" style="text-align:center;border:1px solid black;">'.getLang2($lang,'payment_date').'</th>';
$html.='<th width="100px;" style="text-align:center;border:1px solid black;">'.getLang2($lang,'paid_amount').'</th>';
$html.='<th width="70x;" style="text-align:center;border:1px solid black;">'.getLang2($lang,'invoice_date').'</th>';
$html.='<th width="60x;" style="text-align:center;border:1px solid black;">'.getLang2($lang,'vat').'</th>';
$html.='<th width="100x;" style="text-align:center;border:1px solid black;">'.getLang2($lang,'paid_vat_excluded_2').'</th>';
$html.='</tr>';
...
foreach($payments as $item) { 
	...
	if(@$item->paid_amount_vat_included != 0.00){
		$val = floatval(@$item->paid_amount_vat_included);
		$paid_amount_vat_included = 
	...
	if(@$item->paid_amount_vat_excluded != 0.00){
		$val = floatval(@$item->paid_amount_vat_excluded);
